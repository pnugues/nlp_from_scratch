...
Collobert et al. proposed two main architectures: One with two linear layers and the other adding a convolutional layer. They used the simpler feed-forward network with two layers for the part-of-speech (POS) tagger, chunker, and named entity recognition and the second network for semantic role labeling. I reimplemented the first network only. To optimize the sequences, Collobert et al. added a conditional random field (CRF) layer to their first system. This corresponds to the two main notebooks: The first one, `pos_ff_embs.ipynb`, with the feed-forward sequence and the second one, `pos_ff_embs_crf.ipynb`, where I added the CRF. A third notebook, `pos_ff_embs_crf_senna.ipynb`, contains a reimplementation of the CRF layer with the method Collobert et al. used to compute the $\delta$, see Sect. 3.3.2. This notebook is purely stochastic and cannot handle batches.
