...
Collobert et al. proposed two main architectures: One with two linear layers and the other adding a convolutional layer. They used the simpler feed-forward network with two layers for the part-of-speech (POS) tagger, chunker, and named entity recognition and the second network for semantic role labeling. I reimplemented the first network only. To optimize the sequences, Collobert et al. added a conditional random field (CRF) layer to their first system. This corresponds to the two main notebooks: The first one, `pos_ff_embs.ipynb`, with the feed-forward sequence and the second one, `pos_ff_embs_crf.ipynb`, with the CRF. I used the `torchcrf` module to compute the sequence probability. A third notebook, `pos_ff_embs_crf_senna.ipynb`, contains a class reimplementing the CRF layer with the method Collobert et al. used to compute the $\delta$ paramaters, see Sect. 3.3.2 in their paper. This notebook is purely stochastic and cannot handle batches (contrary to `torchcrf`).
...
## Experimental Setup
The datasets have different structures. EWT and CoNLL 2003 have a validation set, but CoNLL 2000 does not. Collobert et al. do not provide details on the fitting and evaluation conditions. When CoNLL 2000 was released, experimental protocols were not yet well standardized, which is reflected in its structure.

Collobert et al. used the Penn Treebank for POS tagging, CoNLL 2000, and CoNLL 2003. They did not describe precisely how they selected their model and how (or if) they used the CoNLL 2003 validation set. Attardi, in its [`deepnl`](https://github.com/attardi/deepnl?tab=readme-ov-file#benchmarks) reimplementation, merges the training and validation sets and removes the title lines. 

As there is no validation set in CoNLL 2000, in my experiments, I used the test set of both CoNLL 2000 and CoNLL 2003 as validation set and I included, similarly to Attardi, the validation set of CoNLL 2003 in the training set. The results I report are then not equivalent to those of Collobert et al. For the EWT, I report the classical training, validation, and test results.
 ...
 * The convergence is relatively quick. We can reach optimal results with less than 25 epochs;
 * The value of the learning rate is important. I tested a few values, but Senna's 0.01 seems to yield the best results;
 * A batch size of one results in better scores. Batches of four samples or more speed up training of course;
 * The Adagrad is better than the plain stochastic gradient descent (SGD) available from PyTorch. Adadelta is very slow, but the convergence is regular;
 ...
 * The embedding initialization has a significant impact on the scores. The centered uniform distribution divided by 10 has a better performance than a comparable normal distribution. An  initialization with the default normal distribution leads to quite disappointing results;
